...
def derive_beat_visuals(grok: GrokClient, narration: str, category_label: str,
    ...
    sys = (
        ...
        "  - Adult-sized humanoid skeleton (~7.5 head heights, real adult height "
        "— NOT chibi, NOT cartoon-mascot, NOT Funko).\n"
        "  - Off-white slightly luminous bone, hollow empty black eye sockets "
        "(no pupils, no dots, no glow).\n"
        "  - Stands at human scale alongside real people / props.\n\n"
        ...
        "sheet below, COPY that subject's outfit verbatim. The skeleton wears the "
        "FULL costume like a real person would: opaque armor/clothing covering "
        "the entire body — chest, torso, arms, legs, boots. The skull is visible "
        "above the collar OR through an open helmet faceplate; bony hands appear "
        "at gauntlet/sleeve edges. NEVER make the costume see-through or X-ray; "
        "no ribcage / spine / pelvis showing through a chestplate.\n"
        "     If the beat is narrator-style or the skeleton is just BEING "
        "(walking down a hallway, standing in a classroom), set outfit to "
        "'no clothing' — skeleton renders as full naked anatomical bone (ribcage "
        "+ spine + pelvis visible). That's the canonical look for non-costume "
        "beats.\n"
