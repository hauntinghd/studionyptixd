...
export default function LandingPage({ onNavigate }: { onNavigate: PageNav }) {
    const { session, signInWithGoogle, topupPacks, checkout, defaultMembershipPlanId } = useContext(AuthContext);
    ...
    const startFreeTrial = () => {
        ...
        void (async () => {
            ...
            const err = await checkout(defaultMembershipPlanId || 'studio_pro_1k', { trial: true });
            setTrialLoading(false);
            if (err) setTrialError(err);
        })();
    };
                ...
                <div className="mx-auto grid min-h-[calc(100vh-3.5rem)] max-w-7xl items-center gap-10 px-4 pb-14 sm:min-h-[calc(100vh-4rem)] sm:px-6 sm:pb-16 lg:grid-cols-[0.95fr,1.05fr]">
                    <div className="max-w-3xl">
                        ...
                        <div className="mt-7 flex flex-col gap-3 sm:mt-8 sm:flex-row">
                            ...
                            >
                                {trialLoading ? 'Opening trial...' : session ? 'Start free trial' : 'Start creating'}
                        ...
                        {trialError && <p className="mt-3 text-sm text-amber-200">{trialError}</p>}
