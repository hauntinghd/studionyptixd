"""CrypticScience — $10K bank rule + Social Security (verified Rook build).

Graves search hook + Graham Stephan scenarios + source-proof motion graphics.

Run:
  python long_form/build_cryptic_ctr_ss_rook.py
  python long_form/build_cryptic_ctr_ss_rook.py --preview
  python long_form/build_cryptic_ctr_ss_rook.py --assemble-only
"""
...
from long_form.cryptic_ctr_ss_rook_beats import (
    AVATAR_MOTION,
    BEATS,
    HOST_IMAGE_PROMPT,
...
from long_form.cryptic_ctr_ss_script import SCENES
from long_form.cryptic_verified_lanes import DISCLAIMER, UPLOAD_TAGS_BASE

_AVATAR_CHAPTERS = {s["id"]: s["chapter"] for s in SCENES}
...
OUT = Path(r"D:/recaps/cryptic_science/ctr_ss_rook_v1")
...
def _write_upload_pack(
    ...
    *,
    ...
    sources_path = ROOT / "long_form" / "cryptic_ctr_ss_sources.json"
    ...
    meta = {
        **meta,
        ...
        "upload_desc": (
            "The $10,000 cash reporting rule explained — plus what Social Security's 2026 COLA "
            "means if you deposit cash. Primary sources only. Updated through May 27, 2026.\n\n"
            f"PRIMARY SOURCES:\n{primary_links}\n\n"
            f"CHAPTERS:\n{chapters}\n\n"
            f"{DISCLAIMER}\n\n"
            f"{UPLOAD_TAGS_BASE}"
        ...
        "upload_tags": (
            "10000 dollar bank rule, CTR, currency transaction report, FinCEN, IRS, "
            "Social Security COLA 2026, Medicare, structuring, bank deposit, retirement, "
            "seniors, tax tips, verified explainer, CrypticScience"
        ),
        "pinned_comment": (
            "Next verified breakdown — IRMAA premiums, May 19 banking executive order follow-up, "
            "or 1099-K rules? Top comment picks it."
        ),
...
def _assemble_final(
    *,
    ...
    final_name = "CrypticScience_CTR_SS_Rook_PREVIEW.mp4" if preview else "CrypticScience_CTR_SS_Rook_v1.mp4"
    ...
    *,
    ...
    if assemble_only:
        ...
        meta = {
            ...
            "reference": "https://youtu.be/UfLIPjy0F_4",
