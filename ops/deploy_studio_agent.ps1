...
'@
$activeOutput = @(($activeProbe -replace "\r","") | & $ssh @sshArgs "tr -d '\r' | bash -s")
...
$stageCommand = "tr -d '\r' | bash -s -- '$gitSha' '$buildId' '$repoUrl'"
...
as_root bash "${release_dir}/ops/contabo/deploy.sh" \
  ...
  --candidate "${candidate}"
'@
$activateCommand = "tr -d '\r' | bash -s -- '$gitSha' '$buildId'"
