...
def _full():
    os.environ["STUDIO_APP_DATA_DIR"] = str(ROOT)
...
def run_wait(ep, inp, timeout=900):
...
tid = gql("mutation($input: SaveTemplateInput!) { saveTemplate(input: $input) { id } }", {"input": {
    "name": "cliplab-inf-v3", "imageName": "runpod/pytorch:2.1.0-py3.10-cuda11.8.0-devel",
    "dockerArgs": START, "ports": "", "volumeInGb": 0, "containerDiskInGb": 50, "isServerless": True, "env": [],
}})["data"]["saveTemplate"]["id"]

EP = gql("mutation($input: EndpointInput!) { saveEndpoint(input: $input) { id } }", {"input": {
    "name": "cliplab-inf-v3", "templateId": tid,
...
for k, v in files.items():
    if sz + len(v) > 2_000_000 and batch:
        print("push", run_wait(EP, {"task": "bootstrap_weights", "files": batch}).get("output"))
        batch, sz = {}, 0
    batch[k] = v
    sz += len(v)
if batch:
...
print("health", json.dumps(run_wait(EP, {"task": "health"}), indent=2))
print("score", json.dumps(run_wait(EP, {"task": "score_segments", "prompt": "hot takes", "segments": [
